...
def ls(options, command_input, options_input):
    ...
    if exclusive_option == '':
        ...
        files = [item for item in items if item.is_file() and item.name == options_input]
    
    elif exclusive_option == 'e':
        files = [item for item in items if item.is_file() and options_input in item.suffixes]
